 ...
 *
 */
public class FriendsSuggestionsScreen extends ListActivity {
	...
	public void onResume() {
		...
		String result = mss.GetFriendsSuggestions(auth);
		...
		if(result.equals(""))
		{
			...
			try{
				...
				for (int i=0;i<json.getJSONArray("friends").length();i++){
					String friend = ((JSONObject)json.getJSONArray("friends").get(i)).getString("first_name")+" "+((JSONObject)json.getJSONArray("friends").get(i)).getString("last_name");
					friends.add(friend);
				}
			}  
	...
	protected void onListItemClick(ListView l, View v, int posicao, long id)
	{
		String username = "";
		try {
			username = ((JSONObject)json.getJSONArray("friends").get(posicao)).getString("username");
